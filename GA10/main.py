...
for i in range(int(pop)):  # Shuffles the elements in the vector n times and stores them
    sol = rd.sample(X1, len(X1)) #create random solutions to generate the initial population
    n_list = np.vstack((n_list, sol)) #np.vstack concatenates arrays along rows
...
tkl1 = np.empty((0, len(X1)))  # keep every generations mutant children
tkl2 = np.empty((0, len(X1)))  # keep every generations mutant children
...
for i in range(gen):
    ...
    for j in range(int(pop/2)):
        ...
        #CROSSOVER
        Child_1 = [0 for i in range(len(X1))] #create list for child1
        Child_2 = [0 for i in range(len(X1))] #create list for child2
        cros_rnd = np.random.rand() #choose random number between 0-1 to decide will the crossover happen or not
        ...
        if Ran_Mut_1 < p_m: #if mutation prob. is bigger, mutation happens
            ...
            A1 = Ran_Mut_2
            print("point index1", A1)
            A2 = Ran_Mut_3
            print("point index2", A2)
            while A1 == A2:
                A2 = np.random.randint(0, len(X1))
                print("changed index", A2)

            if A1 < A2: #if second index is bigger than first index
                Child_value = Child_1[A1] #value of the selected index
                Child_value2 = Child_1[A2]  #value of the selected index
                ...
                Child_value = Child_1[A2]  #value of the selected index
                Child_value2 = Child_1[A1]  #value of the selected index
        ...
        if Ran_Mut_1 < p_m:  #if mutation prob. is bigger mutation happens
            ...
            if A1 < A2: #if second index is bigger than first index
                Child_value = Child_2[A1]  #value of the selected index
                Child_value2 = Child_2[A2]  #value of the selected index
                A2 = A2 + 1 #[1:3] to include 3
                Rev_1 = Child_2[:]
                ...
                Child_value = Child_2[A2]  #value of the selected index
                Child_value2 = Child_2[A1]  #value of the selected index
                A1 = A1 + 1 #[1:3] to include 3
                Rev_1 = Child_2[:]
        ...
        for f in Mutated_Child_1:
             MC1.append(int(f))
        asr1 = np.array(MC1)[np.newaxis].astype(str)
        tkl1 = np.vstack((tkl1, asr1))
        print("asr1",asr1)
        All_in_Generation_X_1_1_temp = np.array(MC1)[np.newaxis]
        alll = (np.column_stack((Total_Dist_Mut_1.astype(str), asr1)))
        print("alll",alll)
        All_in_Generation_X_1 = np.vstack((All_in_Generation_X_1, alll))
        ...
        for f in Mutated_Child_2:
            MC2.append(int(f))
        asr2 = np.array(MC2)[np.newaxis].astype(str)
        tkl2 = np.vstack((tkl2, asr2))
        print("asr2",asr2)
        All_in_Generation_X_2_1_temp = np.array(MC2)[np.newaxis]
        alll2 = (np.column_stack((Total_Dist_Mut_2.astype(str), asr2)))
        print("alll2",alll2)
        All_in_Generation_X_2 = np.vstack((All_in_Generation_X_2, alll2))
        ...
        New_Population = np.vstack((tkl1, tkl2))
    ...
    t = 0
    R_Final = []
    #substitute best with the worst
    ...
    print("Final_Best",Final_Best_in_Generation_X)
    ...
    for i in Save_Best_in_Generation_X:
        ...
        t = t + 1
    Worst_Final_in_Generation_X = R_22_Final[np.newaxis]
    print("Final_Worst",Worst_Final_in_Generation_X)

    Worst_1 = np.where((Save_Best_in_Generation_X == Worst_Final_in_Generation_X).all(axis=1))
    print("worst route index in Save_Best_in_Generation_X",Worst_1)
    ch = Final_Best_in_Generation_X[0:, 1:].tolist()
    print("Final_Best",ch)
    print(".........................................")
    #change the worst one with the best one
    New_Population[Worst_1] = ch
    print("New_Population",New_Population)
