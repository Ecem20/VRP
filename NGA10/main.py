...
n_list = np.array([rd.sample(X1, len(X1)) for i in range(int(pop))])
...
for i in range(gen):
    ...
    for j in range(int(pop/2)):
        ...
        asr1 = Mutated_Child_1[np.newaxis].astype(str)
        print("asr1", asr1)

        alll = (np.column_stack((Total_Dist_Mut_1, asr1)))
        print("alll",alll)
        ...
        asr2 = Mutated_Child_2[np.newaxis].astype(str)
        print("asr2", asr2)
        alll2 = (np.column_stack((Total_Dist_Mut_2, asr2)))
        print("alll2",alll2)
