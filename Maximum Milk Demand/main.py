...
cap = 2500
speed = 70
k = round(speed / 60) #dakikada alınan yol
...
for i in range(S):
    ...
    if (tk - total_Distance >= 0 and cap-totalMilk >= 0):
        ...
        while next_loc != -1:  # Continue while there are available locations
            max_val = -1
            next_loc = -1
            for i in range(len(B)):
                if mask[i] and milk(i) > max_val and milk(i) <= cap - totalMilk:
                    max_val = milk(i)
                    next_loc = i
        ...
        print("Average turnaround time:", round(((total_Distance) /(k*60))), "hr.")
