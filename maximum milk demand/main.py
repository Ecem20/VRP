...
for i in range(S):
    ...
    for i in range(len(mask)):
        if mask[i] and B[i] > max_val:
            ...
            next_loc = i
    mask[next_loc] = False
    total_Distance += dist(last, next_loc+1)  # total distance #bir önceki,next_loc gideceği location
    ...
    if (tk - total_Distance >= 0 and cap-totalMilk >= 0):
        ...
        mask[next_loc] = True
        totalMilk -= milk(next_loc) # bir sonraki seçeceği marketin süt miktarını toplam süt miktarından çıkar
